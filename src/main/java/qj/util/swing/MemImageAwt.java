...
public class MemImageAwt implements Serializable {

	public final Color[][] data;
	
//	public static MemImageAwt EMPTY = new MemImageAwt(new int[0][0]);

	public MemImageAwt(Color[][] data) {
		this.data = data;
	}

	public MemImageAwt(BufferedImage image) {
		this(ImageUtil.getDataAwt(image));
	}
//
...
//		return null;
//	}
//	public Point indexOf(MemImageAwt target, F2<Color,Color,Boolean> colorF) {
...
//	}
//
//	public boolean equalsImage(MemImageAwt target,
...
//
//
//	public boolean equals(MemImageAwt target) {
//		return ImageUtil.equalsImageI(target.data, this.data, 0, 0);
//	}
//	public boolean equals(MemImageAwt target, int tolerance) {
...
//	}
//	
//	public boolean equals(MemImageAwt target,int tolerance, Color transColor) {
...
//	}
//
//	public MemImageAwt subImage(Rectangle rect) {
...
//		}
//		
//		return new MemImageAwt(data);
//	}
//
//
//	public MemImageAwt subImage(boolean[][] mines) {
...
//
//	}
//	public Collection<Point> allIndexOf(MemImageAwt target, F2<Color,Color,Boolean> colorF) {
////		int tolerance = 0;
//		LinkedList<Point> ret = new LinkedList<Point>();
//		int width = target.getWidth();
...
//	}
//
//	public MemImageAwt replaceColor(Color toColor, F1<Color, Boolean> colorF) {
//		MemImageAwt t = duplicate();
//		for (int y = 0; y < getHeight(); y++) {
//			for (int x = 0; x < getWidth(); x++) {
...
//	}
	...
	public static MemImageAwt load(String path) {
		File inputFile = new File(path);
		return load(inputFile);
	}

	public static MemImageAwt load(File inputFile) {
		try {
			return new MemImageAwt(ImageIO.read(inputFile));
		} catch (IOException e) {
...
//	}
//
//	public MemImageAwt transform(F2<Point, Color, Color> f2) {
//		MemImageAwt duplicate = duplicate();
...
//		MemImageAwt img = new MemImageAwt(image);
...
//	}
	...
	public MemImageAwt magnify(final double magnify) {
		final MemImageAwt old = this;
		return new MemImageAwt(data) {

			@Override
			public int getWidth() {
				return (int) Math.ceil(magnify * old.getWidth());
			}

			@Override
			public int getHeight() {
				return (int) Math.ceil(magnify * old.getHeight());
			}

			@Override
			public Color getColor(int x, int y) {
				return old.getColor(
						(int)Math.floor((double)x/ magnify),
						(int)Math.floor((double)y/ magnify)
						);
			}
		};
	}
	
	public BufferedImage toImageAwt() {
		return subImage(0, 0, getWidth(), getHeight());
	}
...
//		long start = System.currentTimeMillis();
		...
		for (int y1 = 0; y1 < h; y1++) {
			for (int x1 = 0; x1 < w; x1++) {
				g2.setColor(getColor(x1 + x, y1 + y));
